...
The original deep learning model in~\cite{chen2023global} was developed for ground irradiance estimates at a single location, which is centered in the domain of satellite images with $11\times11$ pixels. As shown in Fig.~\ref{fig:map}(a), the target is one of the SURFRAD stations (e.g., TBL), and the satellite images cover the same region with the surrounding locations. The target station can be anywhere as long as there are on-site irradiance measurements available. Following the same methodology, the target is expanded from one station to the $11\times11$ surrounding area with 121 locations. As shown in Fig.~\ref{fig:map}(b), selected spectral satellite images of GOES-16 (see Table~\ref{tab:sat}) with the size of $21\times21$ pixels are used to obtain the GHI estimates for the whole region ($11\times11$ pixels) via the pre-trained deep learning model. Specifically, datasets in 2019 are used to develop the pre-trained deep learning model for GHI estimation at a single SURFRAD station using spectral satellite images ($11\times11$ pixels) and ground measurements. The pre-trained deep learning model is then applied to obtain GHI estimates in 2020 for the whole surrounding region~(121 locations) by shifting an $11\times11$-pixel window over the larger satellite images ($21\times21$ pixels) as illustrated in Fig.~\ref{fig:map}(b).
